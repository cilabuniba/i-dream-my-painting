...
from .transformer_2d import FreestyleTransformer2DModel
...
def get_down_block(
    ...
    elif down_block_type == "FreestyleCrossAttnDownBlock2D":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for FreestyleCrossAttnDownBlock2D")
        return FreestyleCrossAttnDownBlock2D(
            num_layers=num_layers,
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            out_channels=out_channels,
...
def get_mid_block(
    ...
    elif mid_block_type == "FreestyleUNetMidBlock2DCrossAttn":
        return FreestyleUNetMidBlock2DCrossAttn(
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            temb_channels=temb_channels,
            dropout=dropout,
...
def get_up_block(
    ...
    elif up_block_type == "FreestyleCrossAttnUpBlock2D":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for CrossAttnUpBlock2D")
        return FreestyleCrossAttnUpBlock2D(
            num_layers=num_layers,
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            out_channels=out_channels,
...
class FreestyleCrossAttnDownBlock2D(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(
        ...
        if freestyle_layout_mask is not None:
            freestyle_layout_mask = F.interpolate(freestyle_layout_mask.to(torch.float32), size=hidden_states.shape[-2:], mode="nearest").to(torch.long)
...
class FreestyleUNetMidBlock2DCrossAttn(nn.Module):
    def __init__(
        self,
        in_channels: int,
        temb_channels: int,
...
class FreestyleCrossAttnUpBlock2D(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
